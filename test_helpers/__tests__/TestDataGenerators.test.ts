...
describe("TestDataGenerators suite", () => {
  ...
  describe("Date generators", () => {
    test.prop([fc.gen()])("fastCheckGenerateValidJSDateObject", (fcGen) => {
    ...
    test.prop([fc.gen()])(
      "fastCheckGenerateValidJSDateObjectStringWithIntegerDateParts",
      (fcGen) => {
        const [actualDateString, actualDateParts]: [
          string,
        ...
        expect(convertDatePartsIntoStringDate(actualDateParts)).toBe(
          actualDateString,
        );
      },
    );

    test.prop([fc.gen()])("fastCheckJSDateObjectsRangeGenerator", (fcGen) => {
      const [actualDates, actualHoursAndMinutes]: [Array<Date>, Array<number>] =
  ...
  describe("fastCheckTimestampObjectGenerator", () => {
    ...
    test.prop([fc.gen()])(
      "fastCheckTimestampWithStartTimeObjectGenerator",
      (fcGen) => {
        const [actualTimestampObject] =
    ...
    test.prop([fc.gen()])("fastCheckTimestampObjectRangeGenerator", (fcGen) => {
      const [actualTimestampObjects]: [Array<Date>, Array<number>] =
        fastCheckTimestampObjectRangeGenerator(fcGen);
      assertYearMonthDayAndDayOfWeekofArrayOfTimestampObjects(
        actualTimestampObjects,
      );
    });
  });
  ...
  describe("Array generators suite", () => {
    test.prop([fc.array(fc.string(), { minLength: 1 }), fc.gen()])(
      "fastCheckShuffledSubarray",
      (testArray, fcGen) => {
        const actualShuffledSubarray: Array<string> = fastCheckShuffledSubarray(
          testArray,
          fcGen,
        );
        convertArraysToSetsAndAssertIsSubset([
    ...
    test.prop([fc.array(fc.string(), { minLength: 1 }), fc.gen()])(
      "fastCheckShuffledArray",
      (testArray, fcGen) => {
        const actualShuffledArray: Array<string> = fastCheckShuffledArray(
          fcGen,
          testArray,
        );
        convertArraysToSetsAndAssertStrictEqual([
  ...
  describe("Capture template generators suite", () => {
    test.prop([fc.gen()])("fastCheckGenerateCaptureTemplateString", (fcGen) => {
      const actualCaptureTemplateString: string =
        fastCheckGenerateCaptureTemplateString(fcGen);
      const actualExpandedStringParts: Set<string> = new Set(
        split(" ", actualCaptureTemplateString),
      );
      assertIsSubset(
        actualExpandedStringParts,
        TESTORGCAPTURETEMPLATESWITHCURSORSET,
      );
    });
    ...
    test.prop([fc.gen()])(
      ...
      (fcGen) => {
        ...
        const actualExpandedStringPartsSet: Set<string> = new Set(
          split(" ", actualCaptureTemplateString),
        );
    ...
    test.prop([fc.gen(), fc.integer({ min: 2, max: 100 })])(
      ...
      (fcGen, testKeysCount) => {
        ...
        expect(Object.keys(testCustomVariablesObject).length).toEqual(
          testKeysCount,
        );
        expect(Object.values(testCustomVariablesObject).length).toEqual(
          testKeysCount,
        );
      },
    );
    ...
    test.prop([fc.gen(), fc.integer({ min: 2, max: 100 })])(
      "fastCheckGenerateCaptureTemplateStringWithCustomVariablesAndCursor",
      (fcGen, testKeysCount) => {
        const [actualCaptureTemplateString, testCustomVariablesObject] =
          fastCheckGenerateCaptureTemplateStringWithCustomVariablesAndCursor(
            fcGen,
            testKeysCount,
          );
        ...
        const expectedSuperset = pipe([
          Object.keys,
          concat(TESTORGCAPTURETEMPLATESWITHCURSOR),
          convertToSet,
