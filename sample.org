# org-everywhere will not reflow if there's hard-wrapped content
...
This header has some description text and a couple subheaders. Look through the next few top level headers to learn more about org-everywhere.
** This is a subheader
...
*** DONE Check out org-everywhere
*** DONE Follow the org-everywhere tutorial
This interactive tutorial helps you get to know org-everywhere really quickly.
*** TODO Learn how to use TODOs in org-everywhere
...
org-everywhere also supports [[http://orgmode.org/manual/Workflow-states.html#Workflow-states][custom todo states]] (if declared at the top of the file). Swipe right on this header a few times.
...
** Undo / Redo
   ...
   headerbar.
* Tables
org-everywhere has native support for viewing and editing tables.
...
org-everywhere has native support for displaying plain lists and checkboxes.
...
* Timestamps
org-everywhere has native support for displaying and editing [[https://orgmode.org/manual/Timestamps.html#Timestamps][timestamps]].
...
org-everywhere supports parsing and preserving the minimum/maximum range timestamps. Alas, only the minimum range is handled as a [[https://orgmode.org/manual/Repeated-tasks.html][repeated task]]. There's no UI for the maximum range, it can be edited as raw text. Also, there is no UI to show how well a habit has been exercised.

* Automatic/Implicit links

org-everywhere recognizes various types of hyperlinks automatically which Emacs Org mode would not necessarily do. That makes sense, because mobile devices, or browsers, enable a different feature set.

For example, when you read this in org-everywhere, then the text [[https://org-everywhere.org]] will implicitly be rendered as a clickable link (as it would be in Emacs Org mode). 

* Property lists
org-everywhere has native support for viewing and editing [[https://orgmode.org/guide/Properties.html][property lists]]. To bring up an editor, expand the ~PROPERTIES~ drawer below and tap on any of the properties.
** Example
:PROPERTIES:
:callsign: Maverick
:eyewear: Aviators
...
:END:
* Planning
org-everywhere has native support for adding and editing DEADLINE and SCHEDULED items. It also supports [[https://orgmode.org/manual/Repeated-tasks.html][repeaters]] and [[https://orgmode.org/manual/Deadlines-and-scheduling.html#Deadlines-and-scheduling][delays]]. Check out these examples:
...
* Capture
org-everywhere supports something like [[https://orgmode.org/manual/Capture.html][Org capture]] in the form of customizable, quickly accessible buttons for creating new headers.
...
=TODO check out org-everywhere|orgmode=
...
You probably noticed that org-everywhere provides suggestions for your filter. After space, =-=, =:=, and =|= you can tap on the completion – no need to type the tag, property, etc.
...
* Agenda
org-everywhere has a basic agenda view that you can access by tapping the calendar button at the bottom of the page.
...
** TODO Check out the org-everywhere agenda view
DEADLINE: <2018-09-10 Mon>
** TODO Install org-everywhere to the homescreen on my mobile phone
...
** This entry also only shows on exactly one day
   <2020-02-17 Mon>
* Syncing
org-everywhere pulls down your org files from Dropbox, GitLab, or WebDAV. Click the "Sign in" button in the upper right hand corner to sign in with either of them and authenticate org-everywhere.

** Backups
The first time you push changes from org-everywhere back up to your chosen sync service, org-everywhere will make a backup of the original file first. It'll be named {your-file-name}.org-everywhere-bak. Dropbox keeps a full version history of your files for you, but this is an additional precaution in case something goes wrong pushing the file back up.
...
* Multiple files
org-everywhere supports using multiple files.
...
* org-everywhere operates completely client side
You don't log in to org-everywhere directly because org-everywhere doesn't have a back end - it operates completely client side using Dropbox, GitLab, or WebDAV as back-ends for storage.
* Capture URL params and Siri support
org-everywhere supports a flexible mechanism for capturing using URL parameters. 

Alternatively, you can take advantage of the URL parameters yourself to build your own custom capture mechanism. 
