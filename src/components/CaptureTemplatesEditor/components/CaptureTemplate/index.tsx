...
export default ({
  ...
  const handleOrgFileAvailabilityChange =
    (orgFileAvailabilityIndex) => (event) =>
      onFieldPathUpdate(
        template.get("id"),
  ...
  const handleAddNewHeaderPath = () =>
    onAddNewTemplateHeaderPath(template.get("id"));
  ...
  const renderIconField = (template) => (
    <div className="capture-template__field-container">
      ...
      <div className="capture-template__help-text">
        Instead of a letter, you can specify the name of any free Font Awesome
        icon (like lemon or calendar-plus) to use as the capture icon. You can
        search the available icons{" "}
  ...
  const renderOrgFileAvailability = (template) => (
    <div className="capture-template__field-container">
      ...
      <div className="capture-template__help-text">
        You can make this capture template available in all org files, or just
      ...
      >
        <div className="multi-textfields-container">
          {template.get("orgFilesWhereAvailable").map((orgFilePath, index) => (
            <div
              key={`org-file-availability-${index}`}
      ...
      <div className="capture-template__field-container">
        <div className="capture-template__field">
          ...
          <select onChange={updateField("file")} style={{ width: "90%" }}>
            {(loadedFilePaths.filter(
              (path) => (path === template.get("file", "")).length,
            ) !== 0
              ? loadedFilePaths
              ...
              <option
                key={path}
                value={path}
                selected={path === template.get("file")}
              >
                {path}
              </option>
            ))}
        ...
        <div className="capture-template__help-text">
          By default the file opened when capturing is the capture target.
          Select a specific file if you want this template to always capture to
          that file. Make sure the file is loaded for it to be selectable here.
          You might also consider to set the file to load on startup in the file
          settings so it's always available.
        </div>
      </div>
    );
  };
  ...
  const renderHeaderPaths = (template) => (
    <div className="capture-template__field-container">
      ...
      <div className="capture-template__help-text">
        Specify the path to the header under which the new header should be
        filed. One header per textfield.
      </div>

      <div className="multi-textfields-container">
        {template.get("headerPaths").map((headerPath, index) => (
          <div
            key={`header-path-${index}`}
            className="multi-textfield-container"
          >
            <input
              type="text"
  ...
  const renderPrependField = (template) => (
    <div className="capture-template__field-container">
      <div className="capture-template__field">
        <div>Prepend?</div>
        <Switch
          isEnabled={template.get("shouldPrepend")}
          onToggle={togglePrepend}
        />
      </div>

      <div className="capture-template__help-text">
        By default, new captured headers are appended to the specified header
        path. Enable this setting to prepend them instead.
      </div>
    </div>
  );
  ...
  const renderTemplateField = (template) => (
    <div className="capture-template__field-container">
      ...
      <div className="capture-template__help-text">
        The template for creating the capture item. You can use the following
        template variables that will be expanded upon capture:
        <ul>
          <li>
          ...
          <li>
            <code>%r</code> - Raw timestamp, date only, no surrounding
            punctation.
            <ul>
              <li>
          ...
          <li>
            <code>%R</code> - Raw timestamp, with date and time, no surrounding
            punctuation.
          </li>
          <li>
  ...
  return (
    <Draggable
      draggableId={`capture-template--${template.get("id")}`}
      index={index}
    >
      {(provided, snapshot) => (
        <div
