    ...
    describe("Renders everything starting from an Org file", () => {
      ...
      describe("Custom todo sequences", () => {
        test("It recognizes custom todo sequences and their DONE state", () => {
          fireEvent.click(getByText("Top level header"));
          expect(
            queryByText("TODO").classList.contains("todo-keyword--done-state"),
          ).toBe(false);
          fireEvent.click(queryByText("TODO"));
          expect(
            queryByText("DONE").classList.contains("todo-keyword--done-state"),
          ).toBe(true);
          expect(
            queryByText("FINISHED").classList.contains(
      ...
      describe("Undo / Redo", () => {
        test("On loading an Org file, both are disabled", () => {
          expect(
            getByTitle("Undo").classList.contains(
              "header-bar__actions__item--disabled",
            ),
          ).toBe(true);
          expect(
            getByTitle("Redo").classList.contains(
              "header-bar__actions__item--disabled",
            ),
          ).toBe(true);
        });
      ...
      describe("Planning items", () => {
        test("deletes planning items", () => {
          fireEvent.click(queryByText("Top level header"));
          fireEvent.click(
            queryByText("A todo item with schedule and deadline"),
          );
      ...
      describe("Agenda", () => {
        test("renders Agenda for an Org file", () => {
          ...
          expect(
            queryAllByText("A todo item with schedule and deadline"),
          ).toBeTruthy();
        });
        ...
        test("Clicking a TODO within the agenda highlights it in the main view", () => {
          ...
          expect(queryByText("Agenda")).toBeTruthy();
          fireEvent.click(
            queryAllByText("A todo item with schedule and deadline")[0],
          );
          expect(queryByText("Agenda")).toBeFalsy();
          expect(
            queryByText("A todo item with schedule and deadline"),
      ...
      describe("TableEditor", () => {
        ...
        test("can insert timestamp in a cell", () => {
          ...
          const expectedDate = getCurrentTimestampAsText({
            isActive: true,
            withStartTime: false,
          });
