...
export default class TablePart extends PureComponent {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (this.props.table !== prevProps.table) {
      this.setState({
        rawCellValues: this.generateCellValueMap(this.props.table),
      });
    }
  }
    ...
    return Map(
      table
        ...
        .map((row) =>
          row
            .get("contents")
            .map((cell) => [cell.get("id"), cell.get("rawContents")]),
        )
        .flatten(),
    );
  }
  ...
  handleTableSelect(tableId) {
    this.props.subPartDataAndHandlers.onTableSelect(
      tableId,
    ...
    return (
      <Fragment>
        <table
          ...
          onClick={() => this.handleTableSelect(table.get("id"))}
        >
          <tbody>
                    ...
                    <td className={className} key={cell.get("id")}>
                      {cell.get("contents").size > 0 ? (
                        <AttributedString
                          ...
                          subPartDataAndHandlers={
                            this.props.subPartDataAndHandlers
                          }
                        />
                      ) : (
                        "   "
