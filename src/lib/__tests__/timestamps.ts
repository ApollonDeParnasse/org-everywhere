...
export const toDateString = Function.prototype.call.bind(
  Date.prototype.toDateString,
);
// new Date(year, monthIndex, day, hours, minutes, seconds)
export const newDate = (args: [number, number, number, number, number]) =>
  new Date(...args);
export const convertDatePartsIntoStringDate = pipe([newDate, toDateString]);

export const convertJSDateObjectIntoTimestampStringWithAngleBrackets = curry(
  (formatter: (date: Date) => string, date: Date): string => {
    return `<${formatter(date)}>`;
  },
);

export const convertDateIntoTimestampStringWithSquareBrackets = curry(
  (formatter: (date: Date) => string, date: Date): string => {
    return `[${formatter(date)}]`;
  },
);
...
export const orgTimestampFormatWithDayNameAndStartTime = format(
  "yyyy-MM-dd eee HH:mm",
); // '2025-06-16 Mon 02:03'

export const [
  convertJSDateObjectIntoActiveOrgTimestampString,
  convertJSDateObjectIntoExtendedActiveOrgTimestampString,
] = map<(date: Date) => string, (date: Date) => string>(
  convertJSDateObjectIntoTimestampStringWithAngleBrackets,
)([orgTimestampFormat, orgTimestampFormatWithDayNameAndStartTime]);
export const [
...
export const convertJSDateObjectIntoTimestampObject = curry(
  (
    { isActive = true, withStartTime = false }: DefaultTimestampOptions,
    date: Date,
  ): Timestamp => {
    const [year, month, day, dayOfWeek] =
      getYearMonthDayAndDayNumberFromJSDateObject(date);
...
export const convertArrayOfJSDateObjectsIntoArrayOfTimestampObjects = map(
  convertJSDateObjectIntoTimestampObject({}),
);

export const convertArrayOfJSDateObjectsIntoArrayOfTimestampObjectsWithStartTimes =
  map(convertJSDateObjectIntoTimestampObject({ withStartTime: true }));

export const getCurrentDateAsTimestampObject = ({
  isActive = true,
  withStartTime = false,
} = {}) =>
  convertJSDateObjectIntoTimestampObject(
...
export const convertTimestampObjectIntoOrgTimestampString = (
  timestamp: Timestamp,
): string => {
  const {
    isActive,
...
export const convertTimestampObjectIntoJSDateObject = (
  timestamp: Timestamp,
): Date => {
...
export const convertArrayOfTimestampObjectsIntoArrayOFJSDateObjects = map(
  convertTimestampObjectIntoJSDateObject,
);

export const convertTimestampObjectIntoJSDateString = (
  timestamp: Timestamp,
): string => {
  const { year, month, day, startHour = 0, startMinute = 0 } = timestamp;
  return convertDatePartsIntoStringDate([
    year,
    month,
    day,
    startHour,
...
export const convertRangeOfJSDateObjectsIntoDurationString = curry(
  (start: Date, end: Date): string => {
    ...
    return pipe([
      ...
      over([
        pipe([head, toString]),
        pipe([tail, trimChars("-"), padCharsStart("0", 2)]),
      ]),
      join(":"),
    ])(minutes);
  },
...
export const millisDuration = (millis: number): string => {
  return convertRangeOfJSDateObjectsIntoDurationString(
    DATEZERO,
    new Date(millis),
  );
};
