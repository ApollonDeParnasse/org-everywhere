...
import {
  convertDateIntoMonthNumberString,
...
describe("Extracting and computing property names and values", () => {
  ...
  test.prop([fc.gen()])("Did we got all properties?", () => {
  ...
  test.prop([fc.gen()])("Handles the case of empty values", () => {
...
describe("Find the headline at the end of the headline-path", () => {
  test.prop([fc.gen()], { numRuns: 0 })(
    "where the headline-path contains template variables as headlines",
    (fcGen) => {
      ...
      const activeTimestampAsHeadline = {
        ...
        titleLine: {
          ...
          rawTitle:
            convertJSDateObjectIntoTimestampStringWithAngleBrackets(testDate),
          tags: [],
        },
        propertyListItems: [],
        rawDescription: "",
