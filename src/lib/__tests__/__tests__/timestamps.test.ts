...
describe("timestamps test suite", () => {
  ...
  test.prop([fc.gen()])("toDateString", (fcGen) => {
  ...
  test.prop([fc.gen()])("convertDatePartsIntoStringDate", (fcGen) => {
    const [expectedDate, testDateParts] =
      fastCheckGenerateValidJSDateObjectStringWithDateParts(fcGen);
    const actualDateString: string =
      convertDatePartsIntoStringDate(testDateParts);
  ...
  test.prop([fc.gen(), fc.constantFrom(...formatters)])(
    "convertJSDateObjectIntoTimestampStringWithAngleBrackets",
    (fcGen, testFunction) => {
      const [testDate, [expectedYear]] =
        fastCheckGenerateValidJSDateObjectStringWithDateParts(fcGen);
      const actualTimestamp: string =
        convertJSDateObjectIntoTimestampStringWithAngleBrackets(
          testFunction,
          testDate,
        );
  ...
  describe("convertJSDateObjectIntoTimestampObject test suite", () => {
    test.prop([fc.gen()])("with nothing selected", (fcGen) => {
      const [testDate, [expectedYear]] =
        fastCheckGenerateValidJSDateObjectStringWithDateParts(fcGen);
      const actualTimestamp: Timestamp = convertJSDateObjectIntoTimestampObject(
        {},
        testDate,
      );
      const [
    ...
    test.prop([fc.gen()])("with isActive set to true", (fcGen) => {
      const [testDate, [expectedYear]] =
        fastCheckGenerateValidJSDateObjectStringWithDateParts(fcGen);
      const actualTimestamp: Timestamp = convertJSDateObjectIntoTimestampObject(
        { isActive: true },
    ...
    test.prop([fc.gen()])("with isActive set to false", (fcGen) => {
      const [testDate, [expectedYear]] =
        fastCheckGenerateValidJSDateObjectStringWithDateParts(fcGen);
      const actualTimestamp: Timestamp = convertJSDateObjectIntoTimestampObject(
        { isActive: false },
    ...
    test.prop([fc.gen()])("withStartTime set to true", (fcGen) => {
      const [testDate, [expectedYear]] =
        fastCheckGenerateValidJSDateObjectStringWithDateParts(fcGen);
      const actualTimestamp: Timestamp = convertJSDateObjectIntoTimestampObject(
        { withStartTime: true },
    ...
    test.prop([fc.gen()])(
      "with isActive and withStartTime set to true",
      (fcGen) => {
        const [testDate, [expectedYear]] =
          fastCheckGenerateValidJSDateObjectStringWithDateParts(fcGen);
    ...
    test.prop([fc.constant(new Date())])(
      ...
      (expectedDate) => {
        ...
        const [actualTimestampYear, actualTimestampIsActiveValue] = over([
          getTimestampObjectYear,
          timestampIsActive,
          getTimestampObjectStartHour,
          getTimestampObjectStartMinute,
  ...
  describe("convertTimestampObjectIntoOrgTimestampString test suite", () => {
    test.prop([fc.gen()])("with BasicActiveTimestampObject", (fcGen) => {
      const [testTimestampObject, [expectedYear]] =
        fastCheckActiveTimestampObjectGenerator(fcGen);
      const actualOrgTimestamp: string =
        convertTimestampObjectIntoOrgTimestampString(testTimestampObject);
    ...
    test.prop([fc.gen()])("with BasicInactiveTimestampObject", (fcGen) => {
      const [testTimestampObject, [expectedYear]] =
    ...
    test.prop([fc.gen()])("with BasicActiveTimestampWithStartTime", (fcGen) => {
      const [testTimestampObject, [expectedYear]] =
        fastCheckActiveTimestampWithStartTimeObjectGenerator(fcGen);
      const actualOrgTimestamp: string =
        convertTimestampObjectIntoOrgTimestampString(testTimestampObject);
      expect(startsWith("<", actualOrgTimestamp)).toBeTruthy();
      expect(endsWith(">", actualOrgTimestamp)).toBeTruthy();
    ...
    test.prop([fc.gen()])("with BasicInactiveTimestampWithStart", (fcGen) => {
      const [testTimestampObject, [expectedYear]] =
  ...
  describe("convertTimestampObjectIntoJSDateString test suite", () => {
    test.prop([fc.gen()])("with start hour and start minute", (fcGen) => {
      const [testTimestampObject, , testDate] =
        fastCheckActiveTimestampWithStartTimeObjectGenerator(fcGen);
      const actualOrgTimestamp: string =
        convertTimestampObjectIntoJSDateString(testTimestampObject);
      const expectedOrgTimestamp: string = toDateString(testDate);
      expect(actualOrgTimestamp).toBe(expectedOrgTimestamp);
    });

    test.prop([fc.gen()])("without start hour and start minute", (fcGen) => {
      const [testTimestampObject, , testDate] =
        fastCheckActiveTimestampObjectGenerator(fcGen);
      const actualOrgTimestamp: string =
        convertTimestampObjectIntoJSDateString(testTimestampObject);
  ...
  describe("convertTimestampObjectIntoJSDateObject", () => {
    test.prop([fc.gen()])("without startTime", (fcGen) => {
      const [
        testTimestamp,
        [expectedYear, expectedMonth, expectedDay, expectedDayNumber],
      ] = fastCheckActiveTimestampObjectGenerator(fcGen);
      const actualDate: Date =
        convertTimestampObjectIntoJSDateObject(testTimestamp);
      const actualDateParts: Array<number> =
        getYearMonthDayAndDayNumberFromJSDateObject(actualDate);
      zipWithAssertEqual(actualDateParts, [
        expectedYear,
        expectedMonth,
        expectedDay,
        expectedDayNumber,
    ...
    test.prop([fc.gen()])("with startTime", (fcGen) => {
      const [testTimestamp, expectedDateParts] =
        fastCheckActiveTimestampWithStartTimeObjectGenerator(fcGen);

      const actualDate: Date =
        convertTimestampObjectIntoJSDateObject(testTimestamp);
      const actualDateParts: Array<number> =
        getYearMonthDayDayNumberHoursAndMinutesFromJSDateObject(actualDate);
      zipWithAssertEqual(actualDateParts, expectedDateParts);
    });
  });

  describe("convertRangeOfJSDateObjectsIntoDurationString", () => {
    test.prop([fc.gen()])("with positive difference", (fcGen) => {
      const [[testStart, testEnd], [expectedHours, expectedMinutes]] =
        fastCheckJSDateObjectsRangeGenerator(fcGen);
      const actualDurationString: string =
        convertRangeOfJSDateObjectsIntoDurationString(testStart, testEnd);
    ...
    test.prop([fc.gen()])("with negative difference", (fcGen) => {
      const [[testStart, testEnd], [expectedHours, expectedMinutes]] =
        fastCheckJSDateObjectsRangeGenerator(fcGen);
      const actualDurationString: string =
        convertRangeOfJSDateObjectsIntoDurationString(testEnd, testStart);
      assert.include(actualDurationString, ":");
  ...
  describe("convertRangeOfTimestampObjectsIntoDurationString", () => {
    test.prop([fc.gen()])("with positive difference", (fcGen) => {
      const [testTimestamps, [[expectedHours, expectedMinutes]]] =
        fastCheckTimestampObjectRangeGenerator(fcGen);
      const actualDurationString: string =
        convertRangeOfTimestampObjectsIntoDurationString(testTimestamps);
  ...
  describe("date arithmetic", () => {
    test.prop([fc.nat()])("millisDuration", (testMilliseconds) => {
      const expectedEndDate: Date = addMilliseconds(
        testMilliseconds,
        new Date(0),
      );
      const actualDurationString: string = millisDuration(testMilliseconds);
      const [actualHours, actualMinutes] = pipe([split(":"), map(parseInt)])(
        actualDurationString,
      );
      const actualEndDate: Date = add(
        { hours: actualHours, minutes: actualMinutes },
        DATEZERO,
      );

      pipe([
        map(getYearMonthDayDayNumberHoursAndMinutesFromJSDateObject),
        spread(zipWithAssertEqual),
      ])([actualEndDate, expectedEndDate]);
    });

    test.prop([fc.nat()])(
      "convertTimeUnitsIntoDurationObject",
      (testMilliseconds) => {},
    );
  });
});
