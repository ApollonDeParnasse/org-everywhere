...
export const hasActiveClock = (header) => {
  ...
  const activeEntries = logBookEntries.filter(
    (entry) => entry.get("start") && !entry.get("end"),
  );
...
const totalTimeLogged = (header) => {
  ...
  const times = logBookEntries.map((entry) =>
    entry.get("start") && entry.get("end")
      ? dateForTimestamp(entry.get("end")) -
        dateForTimestamp(entry.get("start"))
      : 0,
  );
...
export const totalFilteredTimeLogged = (filters, header) => {
  ...
  const times = clocks.map(
    ([start, end]) => dateForTimestamp(end) - dateForTimestamp(start),
  );
...
export const updateHeadersTotalTimeLoggedRecursive = (headers) => {
  ...
  const headersWithtotalTimeLoggedRecursive = headersWithtotalTimeLogged.map(
    (header, index) => {
      const subheaders = subheadersOfHeaderWithIndex(
        headersWithtotalTimeLogged,
        index,
      );
...
export const updateHeadersTotalFilteredTimeLoggedRecursive = (
  filters,
  headers,
) => {
  if (!headers) {
  ...
  const headersWithtotalTimeLogged = headers.map((header) =>
    header.set(
      ...
      totalFilteredTimeLogged(filters, header),
    ),
  );
  const headersWithtotalTimeLoggedRecursive = headersWithtotalTimeLogged.map(
    (header, index) => {
      ...
      return header.set(
        "totalFilteredTimeLoggedRecursive",
        totalTimeLoggedRecursive,
      );
    },
  );
  return headersWithtotalTimeLoggedRecursive;
